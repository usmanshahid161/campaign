...
exports.create = asyncHandler(async (req, res) => {
  const campaign = await campaignsService.createCampaign(req.user.tenantId, req.headers.authorization, req.body, req.user?.userId);
  res.status(201).json({ success: true, data: campaign });
});

exports.update = asyncHandler(async (req, res) => {
  const campaign = await campaignsService.updateCampaign(req.user.tenantId, req.params.id, req.headers.authorization, req.body);
  res.status(200).json({ success: true, data: campaign });
});
...
exports.sendTest = asyncHandler(async (req, res) => {
  const result = await campaignsService.sendTest(req.user.tenantId, req.params.id, req.body);
